...
- ⏩ **`highres_v2` — ✅ PHASE 6 BY-EAR PASSED (2026-07-05), a DEPLOY CANDIDATE, but NOT YET the deployed default:**
  ...
  **triplet band** via `--onset_phase_calib "0,1.0,b_trip"` (3rd element, default 0; `b_trip=0.7` BY-EAR WON, `46a25b4`);
  ...
  gate bug is also FIXED (`dens` frame-fraction was 16th-native → the gate never fired on v2; `conditioning-mechanics
  §7` subdiv fix; offline-confirmed, by-ear pending). ⚠️ Remaining before the swap: by-ear the hold-stream fix, then
  the coordinated version bump. Until the swap, the deployed regime STAYS `highres` + `gen_motif_full_fixed`.
