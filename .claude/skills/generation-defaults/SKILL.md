...
- ⏩ **`highres_v2` — ✅ PHASE 6 BY-EAR PASSED (2026-07-05), a DEPLOY CANDIDATE, but NOT YET the deployed default:**
  ...
  **triplet band** via `--onset_phase_calib "0,1.0,b_trip"` (3rd element, default 0; `b_trip=0.7` BY-EAR WON, `46a25b4`);
  ...
  two-foot-jump veto — `conditioning-mechanics §8d`) is now BUILT + BY-EAR PASSED. ⚠️ STILL OPEN before the swap: a
  hold-stream-gate bug on `freeze=high` v2. Until that lands + the swap, the deployed regime STAYS `highres` +
  `gen_motif_full_fixed`. Groove-radar chaos fix is
