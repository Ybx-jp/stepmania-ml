...
> Everything below is that config. Two things that are NOT it: (a) `generate()`'s bare function defaults — raw
...
> (b) `scripts/generate.py` — the conservative shipped CLI; it has DRIFTED stale (`pattern_temperature=0.7`,
> stamina/breathe off) and is NOT the reference for matching what the user plays. When in doubt, mirror the
> exporter.
...
- **Stale `pattern_temperature` 0.7** copied from `scripts/generate.py`/old scripts instead of the validated 1.0.
