...
| Sequence-aware onset head | [seq-onset-arc.md](seq-onset-arc.md) | ✅ | "when" isolated from "where"; placement is a chart-PRIOR not in audio (wall CLOSED NEGATIVE 4 ways, 06-29) — **but the BUILD RE-OPENED (M1a 06-29):** the FROZEN deployed decoder's `h` already encodes the full signal (conv readout = 0.892 ≡ ceiling, 100%) → fork (A) = cheap frozen-head onset add. **M1b-3 (06-29): note-dropout SCHEDULED SAMPLING BREAKS the drift wall** — the frozen-decoder head free-runs coherently from its own context (run 1.0; tau≈0.56→real density); M1b's "collapse" was partly a tau-transfer calibration artifact. Remaining: calibration cliff + placement QUALITY (gen-time 16th-AUC / by-ear) + `generate()` wiring | onset_frozenh / onset_seqrollout / onset_ss · [[onset-phrase-calibrator]] |
