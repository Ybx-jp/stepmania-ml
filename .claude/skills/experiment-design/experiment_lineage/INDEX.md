...
| Sequence-aware onset head | [seq-onset-arc.md](seq-onset-arc.md) | ✅ | "when" isolated from "where"; placement is a chart-PRIOR not in audio (wall CLOSED NEGATIVE 4 ways, 06-29) — **but the BUILD RE-OPENED (M1a 06-29):** the FROZEN deployed decoder's `h` already encodes the full signal (conv readout = 0.892 ≡ ceiling, 100%) → fork (A) = cheap frozen-head onset add. **M1b drift gate (06-29): teacher-forced head COLLAPSES free-run (control cleared the harness) → own-output SCHEDULED SAMPLING is the mandatory next step (M1b-3)** | sequence_aware_onset_plan / onset_frozenh / onset_seqrollout · [[onset-phrase-calibrator]] |
