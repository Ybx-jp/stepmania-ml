...
def main():
    ...
    train = collect(tr_ds, args.max_len, args.max_train); ev = collect(va_ds, args.max_len, args.eval_songs * 2)[:args.eval_songs]
    ...
    def batches():
        ...
        for i in range(0, len(idx), args.bs):
            ch = [train[j] for j in idx[i:i + args.bs]]; T = max(len(a) for a, _ in ch); B = len(ch)
    ...
    real_run = np.mean([run_mean(y) for _, y in ev]); real_dens = np.mean([y.mean() for _, y in ev])
    ...
    with torch.no_grad():
        for a, y in ev:
            ...
            C0 = corrupt(y, rng)                                   # rough first pass
