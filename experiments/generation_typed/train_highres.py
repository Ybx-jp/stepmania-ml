...
Warm-starts from gen_stage1 (the 41-dim model): load_from_factorized loads every shared tensor; the ONLY
shape mismatch is the audio-encoder first conv (input 41->42). Rather than leave it fresh, we EXPAND it —
copy the 41 trained input channels and zero-init the new 42nd column — so at init the model is identical
to gen_stage1 (the new feature contributes 0) and then learns to use it. warmup_freeze=0 (audio encoder
trains from epoch 0 so the new column can move).

Usage:
    python experiments/generation_typed/train_highres.py --data_dir data/ --audio_dir data/ \
...
def main():
    ...
    # Expand the first conv (41->42 input channels): copy trained channels, zero-init the new column,
    # so the model starts identical to gen_stage1 and learns the new high-res-onset dim from there.
    with torch.no_grad():
        ...
        print(f"expanded {FIRST_CONV}: {tuple(old_w.shape)} -> {tuple(new_w.shape)} (new column zero-init)")
