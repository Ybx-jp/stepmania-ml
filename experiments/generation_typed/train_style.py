#!/usr/bin/env python3
"""
Train reference-chart STYLE conditioning (Step 3) on top of the layered typed generator.

A StyleEncoder reuses the Phase 1 ChartEncoder to map a reference chart's per-frame
occupancy -> a single (B,d) style latent (mean-pooled over time = the bottleneck), which
is added to the conditioning vector alongside difficulty + groove-radar. The bottleneck
lets the latent carry global feel (density, which-panels / jack tendencies) but not the
exact note sequence, so it transfers style rather than copies.

During training the reference IS the target chart (autoencoder-style conditioning); the
bottleneck stops trivial copying. We warm-start from the radar checkpoint and train BOTH
knobs, with INDEPENDENT classifier-free-guidance dropout on radar and on style so each
can be amplified (or used alone) at inference.

Usage:
    python experiments/generation_typed/train_style.py --data_dir data/ --audio_dir data/ \
        --epochs 15 --warmup_freeze 2 --batch_size 8
"""
...
DEFAULT_BPM = 150.0
RADAR_CKPT = "checkpoints/gen_radar/best_val.pt"  # warm-start from the trained radar model
...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_style')
    p.add_argument('--radar_drop', type=float, default=0.15, help='CFG: prob of dropping radar conditioning per batch')
    p.add_argument('--style_drop', type=float, default=0.15, help='CFG: prob of dropping style conditioning per batch')
    return p.parse_args()
...
def main():
    ...
    ck = torch.load(RADAR_CKPT, map_location='cpu', weights_only=False)
    print(f"warm-started {model.load_from_factorized(ck['model_state_dict'])} tensors "
          f"(style_encoder + null_style stay fresh)")
    model.freeze_audio_encoder(True)
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")

    type_weight = torch.ones(NUM_TYPES, device=device)  # calibrated focal; rely on type sampling for hold rate
    ...
    def to_tensors(batch):
        ...
        typ = (states - 1).clamp(min=0)  # (B,T,4)
    ...
    if mlflow_on:
        mlflow.start_run(run_name="gen-style"); mlflow.log_params({'epochs': args.epochs, 'lr': args.lr,
                                                                   'radar_drop': args.radar_drop, 'style_drop': args.style_drop})
    ...
    for epoch in range(args.epochs):
        ...
        for batch in batches(train, args.batch_size, True):
            ...
            optimizer.zero_grad()
            cond_radar = None if rng.random() < args.radar_drop else radar       # CFG: drop radar sometimes
            ref = None if rng.random() < args.style_drop else states             # CFG: drop style sometimes; ref = target chart
            ref_mask = None if ref is None else mask
            ol, pat_l, typ_l = model(audio, states, diff, mask, radar=cond_radar,
                                     reference=ref, reference_mask=ref_mask)
        ...
        with torch.no_grad():
            for batch in batches(val, args.batch_size, False):
                ...
                ol, pat_l, typ_l = model(audio, states, diff, mask, radar=radar,
                                         reference=states, reference_mask=mask)
                o, p, t = losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active)
    ...
    # ---- eval: does the style knob reproduce a reference's density when transferred? ----
    ...
    for i in range(0, len(eval_set), 8):
        ...
        ref = torch.zeros(B, L, NUM_PANELS, dtype=torch.long); ref_mask = torch.zeros(B, L, dtype=torch.bool)
        for b, s in enumerate(batch):
            t = min(len(s['typed']), L); audio[b, :t] = torch.from_numpy(s['audio'][:t]); lengths[b] = t
            diff[b] = s['difficulty']; ref[b, :t] = torch.from_numpy(s['typed'][:t]); ref_mask[b, :t] = True
        gen = model.generate(audio.to(device), diff.to(device), lengths=lengths.to(device),
                             ...
                             reference=ref.to(device), reference_mask=ref_mask.to(device)).cpu().numpy()
        for b, s in enumerate(batch):
    ...
    print("\n" + "=" * 76)
    print("  STYLE-CONDITIONED GENERATOR (reference = matching chart, self-reference sanity)")
    print("=" * 76)
    print(f"  onset_F1 {np.mean(f1s):.3f}  density {np.mean(dens):.3f} (real {target_density:.3f})")
    print("  -> run eval_style_transfer.py for the cross-song style-transfer controllability test")
    print("=" * 76)
    if mlflow_on:
