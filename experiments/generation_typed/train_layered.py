...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_layered')
    return p.parse_args()
...
def main():
    ...
    def to_tensors(batch):
        ...
        return (audio.to(device), states.to(device), mask.to(device), diff.to(device),
                pat.to(device), typ.to(device), active.to(device))
    ...
    if mlflow_on:
        mlflow.start_run(run_name="gen-layered"); mlflow.log_params({'epochs': args.epochs, 'lr': args.lr})
    ...
    for epoch in range(args.epochs):
        ...
        for batch in batches(train, args.batch_size, True):
            audio, states, mask, diff, pat_t, typ_t, active = to_tensors(batch)
            optimizer.zero_grad()
            ol, pat_l, typ_l = model(audio, states, diff, mask)
    ...
    with torch.no_grad():
        for batch in batches(eval_set, args.batch_size, False):
            audio, states, mask, diff, pat_t, typ_t, active = to_tensors(batch)
            _, _, typ_l = model(audio, states, diff, mask)
            pred = typ_l.argmax(-1)
    ...
    for i in range(0, len(eval_set), 8):
        ...
        gen = model.generate(audio.to(device), diff.to(device), lengths=lengths.to(device),
                             onset_threshold=tau, greedy=True, type_sample=True).cpu().numpy()
        for b, s in enumerate(batch):
