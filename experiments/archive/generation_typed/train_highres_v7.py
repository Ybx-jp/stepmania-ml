...
v4 fixed v3's backfire by weighting only POSITIVE 16th notes (recall), not all 16th frames.

H4-v5 (THIS script; right early-stop criterion). v4 reported "early stop @ epoch 4" but best was epoch 1:
val_total is ~88% PATTERN loss, the onset head we change is ~7.5% of it, and even val_onset is dominated
by abundant quarter/8th/empty frames -> the rare 16ths we optimize barely move it. So we were selecting &
stopping on a metric BLIND to the experiment's target. v5 selects on a 16th-PHASE onset F1: at the val-set
operating threshold (quantile at real density), precision AND recall restricted to 16th-phase frames
(t%4 in {1,3}). MAXIMIZE it. This tells us whether 16th capability keeps improving past epoch 1 (more
training helps) or plateaus (epoch 1 is the ceiling -> levers are w16/data/decode, not epochs). Everything
else (model, warm-start, random-init high-res col, phase-weighted loss) is identical to v4 — one change.

H4-v7 (THIS script; ADDITIVE 16th fix — see notes/v7_additive_loss_design.md). v6's ranking loss flooded
16ths and NUKED the 8th groove at fixed density (8th-rate 0.163->0.009). v7 = pure REWEIGHTED BCE: LOW w8
(=1, stop rewarding the over-produced 8ths) + MODEST w16 (=10, no ranking loss), warm-start v4. Selected on
an ADDITIVE per-phase-rate metric (val_phase_err): per-song L1 over quarter+8th+16th RATES vs real, weighted
by real 16th-share -> penalizes 8th-collapse AND 16th-overshoot, focused on chaotic songs (fixes both s16err
flaws). Generation couples density to chaos so 16ths ADD on top (eval_v6_additive.py / export --target_density).
Goal: real-like 8th/16th BALANCE without ruining quarters/8ths; placement stays at the ~0.67 audio ceiling.

H4-v6 (fix 16th UNDER-COMMITMENT — see notes/chaos_retrain_scope.md). Step 0
(diag_16th_commit.py) found the defect is KNOWS-BUT-LOSES: under coherent conditioning the model identifies
real 16ths (AUC 0.67) but scores them BELOW 8ths (p_on 0.475 vs 0.612), so the per-song threshold drops them
(recall 6.5% vs 74% for 8ths) and it substitutes 8ths. v4's per-phase RECALL weighting didn't fix the
RANKING. v6 adds a RANKING/MARGIN loss: push real-16th-note logits above the empty off-beat frames they
compete with for the density budget (so they clear the threshold). Warm-starts v4 (already 42-dim, no conv
expansion). Selection metric is DEPLOYMENT-MATCHED (the val_f16 lesson): per-song 16th-SHARE ERROR vs real,
under coherent conditioning at the per-song density threshold — MINIMIZE. One change from v4: + ranking loss.

Usage:
    python experiments/generation_typed/train_highres_v6.py --data_dir data/ --audio_dir data/ \
        --epochs 15 --batch_size 8 --rank_lambda 1.0 --rank_margin 1.0
"""
...
DEFAULT_BPM = 150.0
WARM_CKPT = "checkpoints/gen_highres_v4/best_val.pt"  # v6: continue from v4 (already 42-dim, high-res engaged)
...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_highres_v7')
    p.add_argument('--cache_dir', default='cache/samples_v3', help='42-dim cache (41 musical + high-res onset)')
    p.add_argument('--w8', type=float, default=1.0,
                   help='onset-loss weight on positive 8th notes (v7: 1.0 = do NOT reward 8ths; v4 used 3 and '
                        'OVER-produced 8ths).')
    p.add_argument('--w16', type=float, default=10.0,
                   help='onset-loss weight on positive 16th notes (v7: modest; v6 stacked w16=15 + ranking '
                        'loss and flooded 16ths/nuked 8ths). The additive metric selects the balanced epoch.')
    p.add_argument('--rank_lambda', type=float, default=0.0,
                   help='weight of the 16th-vs-empty-offbeat RANKING/MARGIN loss (v6: fix knows-but-loses; '
                        'pushes real-16th logits above the empty off-beat frames they compete with). 0 = off.')
    p.add_argument('--rank_margin', type=float, default=1.0, help='logit margin for the ranking loss')
    ...
    p.add_argument('--patience', type=int, default=3, help='early stopping: stop after this many epochs with no val_s16err improvement')
    return p.parse_args()
...
def main():
    ...
    # v6: warm-start from v4 (already 42-dim, high-res feature engaged) -> direct load, NO conv expansion.
    miss, unexp = model.load_state_dict(ck['model_state_dict'], strict=False)
    print(f"warm-started from {WARM_CKPT} (v4, 42-dim; epoch {ck.get('epoch')}, val_total {ck.get('val_total')}); "
          f"missing={len(miss)} unexpected={len(unexp)}")
    if miss:
        print(f"  missing keys (kept at init): {miss[:6]}{'...' if len(miss) > 6 else ''}")
    ...
    def losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active):
        ...
        # v4: weight POSITIVE notes by phase (penalize MISSING rare 8th/16th notes = recall). v3 weighted
        # ALL 16th frames, which are ~50% of frames and mostly EMPTY -> reinforced "no 16th" -> p_on@16th
        # went DOWN. Here negatives stay weight 1; positive 8ths -> w8, positive 16ths -> w16.
        B, T = ol.shape
        t = torch.arange(T, device=ol.device)
        is8 = (t % 4 == 2).unsqueeze(0); is16 = ((t % 4 == 1) | (t % 4 == 3)).unsqueeze(0)
        pos = onset_t.bool()
        phase_w = torch.ones(B, T, device=ol.device)
        phase_w = torch.where(pos & is8, float(args.w8), phase_w)
        phase_w = torch.where(pos & is16, float(args.w16), phase_w)
        o = focal_bce(ol[mask], onset_t[mask], args.focal_gamma, weight=phase_w[mask])
        # v7 = pure REWEIGHTED BCE (no ranking loss; v6's ranking @ fixed density flooded 16ths & nuked 8ths).
        # The ranking term is kept but OFF by default (rank_lambda=0). v7 relies on modest w16 + LOW w8 +
        # density coupling at generation (additive) + the additive per-phase-rate selection metric.
        if args.rank_lambda > 0:
            pos_m = mask & pos & is16; neg_m = mask & (~pos) & (is8 | is16)
            pos_l = ol[pos_m]; neg_l = ol[neg_m]
            if pos_l.numel() > 0 and neg_l.numel() > 0:
                idx = torch.randint(0, neg_l.numel(), (pos_l.numel(),), device=ol.device)
                rank = torch.relu(args.rank_margin - (pos_l - neg_l[idx])).mean()
            else:
    ...
    best = float('inf'); bad = 0; best_path = Path(args.checkpoint_dir) / "best_val.pt"  # MINIMIZE val_phase_err
    if mlflow_on:
        mlflow.start_run(run_name="gen-highres-v7"); mlflow.log_params({'epochs': args.epochs, 'lr': args.lr, 'features': '42-dim + high-res onset', 'warm_start': 'gen_highres_v4', 'w8': args.w8, 'w16': args.w16, 'rank_lambda': args.rank_lambda, 'select_metric': 'val_phase_err (additive, chaotic-weighted)'})

    for epoch in range(args.epochs):
        if epoch == args.warmup_freeze:
            model.freeze_audio_encoder(False); print(f"  [epoch {epoch+1}] unfroze audio encoder")
        model.train(); tr = [0.0, 0.0, 0.0, 0.0]; nb = 0
        for batch in batches(train, args.batch_size, True):
            ...
            o, p, t, rank = losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active)
            (o + p + t + args.rank_lambda * rank).backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0); optimizer.step()
            tr[0] += o.item(); tr[1] += p.item(); tr[2] += t.item(); tr[3] += rank.item(); nb += 1
        model.eval(); v = [0.0, 0.0, 0.0, 0.0]; vnb = 0
        # ADDITIVE selection metric (v7): per-song L1 over ALL THREE phase RATES vs real, WEIGHTED by the
        # song's real 16th-share. Penalizes 8th-collapse AND 16th-overshoot, and the weighting focuses on
        # chaotic songs (calm songs ~0 weight) -- fixes both v6/s16err flaws (16th-only + trivial-song avg).
        perr_w, wsum = 0.0, 0.0
        with torch.no_grad():
            for batch in batches(val, args.batch_size, False):
                audio, states, mask, diff, pat_t, typ_t, active, radar = to_tensors(batch)
                ol, pat_l, typ_l = model(audio, states, diff, mask, radar=radar)  # coherent: each song's own radar
                o, p, t, rank = losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active)
                v[0] += o.item(); v[1] += p.item(); v[2] += t.item(); v[3] += rank.item(); vnb += 1
                onset_t = (states != 0).any(-1)
                for b in range(ol.shape[0]):
                    vm = mask[b]
                    p_b = torch.sigmoid(ol[b][vm]).cpu().numpy(); note_b = onset_t[b][vm].cpu().numpy()
                    nnote = int(note_b.sum()); Tb = len(p_b)
                    if nnote < 8:
                        continue
                    ph = np.arange(Tb) % 4; q = ph == 0; e = ph == 2; s = (ph == 1) | (ph == 3)
                    placed = p_b > float(np.quantile(p_b, 1 - note_b.mean()))   # per-song density threshold
                    if placed.sum() == 0:
                        continue
                    gr = (placed[q].sum() / Tb, placed[e].sum() / Tb, placed[s].sum() / Tb)   # gen phase RATES
                    rr = (note_b[q].sum() / Tb, note_b[e].sum() / Tb, note_b[s].sum() / Tb)    # real phase RATES
                    l1 = sum(abs(g - r) for g, r in zip(gr, rr))
                    w = (note_b & s).sum() / nnote                              # weight = real 16th-share (chaotic-focus)
                    perr_w += w * l1; wsum += w
        v = [x / max(1, vnb) for x in v]; vt = v[0] + v[1] + v[2]
        perr = float(perr_w / wsum) if wsum > 0 else 1.0   # chaotic-weighted additive phase-rate error
        print(f"  epoch {epoch+1}/{args.epochs}  train(o={tr[0]/max(1,nb):.3f} pat={tr[1]/max(1,nb):.3f} typ={tr[2]/max(1,nb):.3f})  "
              f"val(o={v[0]:.3f})  phase_err={perr:.4f}" + ("  *" if perr < best else ""))
        if mlflow_on: mlflow.log_metrics({'val_onset': v[0], 'val_pattern': v[1], 'val_type': v[2],
                                          'val_total': vt, 'val_phase_err': perr}, step=epoch)
        if perr < best:
            best = perr; bad = 0
            torch.save({'model_state_dict': model.state_dict(), 'epoch': epoch, 'val_phase_err': perr, 'val_total': vt}, best_path)
        else:
            bad += 1
            if bad >= args.patience:
                print(f"  early stop @ epoch {epoch+1}: no val_phase_err improvement for {args.patience} epochs"); break

    # ---- eval ----
    print(f"\nLoading best (val_phase_err={best:.4f}) for eval...")
