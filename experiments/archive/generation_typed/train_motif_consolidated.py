#!/usr/bin/env python3
"""
H15 CONSOLIDATED deliverable — gen_motif_local2 (working candle/trill section lever) + the figure token, with
FIGURE-AWARE selection. (The corrected option 1: keep the successful local-motif representation, add figure on
top, but select on generation-time sweep-control — not figure-blind val CE. See notes/h15_hierarchical_findings.md.)

This is train_motif_figure_standalone.py with ONE change: warm-start gen_motif_local2 (KEEP the continuous
per-section motif → candle/trill survive) and feed BOTH motif + figure during training. The standalone test
already showed dropping motif doesn't help figure (the sweep ceiling is the soft per-frame realize, not
redundancy), so there is no reason to sacrifice the working representation. Result = ONE model with radar +
continuous motif (candle/trill) + discrete figure (modest sweep nudge). Selection still on the leak-free
sweep-control metric so the figure lever isn't left under-trained by a figure-blind gauge.

Usage:
    python experiments/generation_typed/train_motif_consolidated.py --data_dir data/ --audio_dir data/ \
...
from src.generation.typed import NUM_PANELS, panels_to_pattern, pair_holds
from src.generation.evaluation import onset_density_metrics
from src.generation.motif_codebook import MotifBasis, figure_token, figure_token_schedule, FIGURE_CLASSES
...
LOCAL2_CKPT = "checkpoints/gen_motif_local2/best_val.pt"   # warm-start = candle/trill section lever (KEEP it)
MOTIF_BASIS = PROJECT_ROOT / "cache/motif_basis.npz"
SWEEP = FIGURE_CLASSES.index("sweep/staircase")
...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_motif_full')
    ...
    p.add_argument('--ctrl_songs', type=int, default=8)
    p.add_argument('--ctrl_len', type=int, default=512)
    p.add_argument('--f1_floor', type=float, default=0.62)
    return p.parse_args()
...
def collect_typed(ds, cap, basis, section):
    ...
    for i in range(len(ds)):
        ...
        sched = local_motif_targets(typed_full, radar, basis, section)
...
def main():
    ...
    basis = MotifBasis.load(MOTIF_BASIS); S = args.section
    ...
    print(f"Collecting typed + motif schedules + figure tokens (section={S})...")
    train = collect_typed(train_ds, args.max_train_len, basis, S)
    val = collect_typed(val_ds, args.max_train_len, basis, S)
    print(f"train={len(train)} val={len(val)}")
    audio_dim = train[0]['audio'].shape[1]; assert audio_dim == 42
    ctrl = val[:args.ctrl_songs]
    ...
    def sweep_frac(ch):
        cnt = Counter(figure_token(ch[i:i + S]) for i in range(0, ch.shape[0], S))
        tot = sum(cnt.values()); return cnt[SWEEP] / tot if tot else 0.0

    @torch.no_grad()
    def control_metric():
        """leak-free sweep control: figure=sweep (motif=null) vs figure=null, realized sweep-frac lift + quality."""
        model.eval(); fr_set, fr_base, f1s = [], [], []
        for s in ctrl:
    ...
    if mlflow_on:
        mlflow.start_run(run_name="gen-motif-full")
        mlflow.log_params({'epochs': args.epochs, 'patience': args.patience, 'warm': 'gen_motif_local2',
                           'mode': 'consolidated motif+figure', 'select': 'sweep-control', 'section': S})
    ...
    print("Eval candle+trill (eval_motif.py --ckpt .../gen_motif_full) AND sweep (eval_figure_control.py) — one model, all levers.")
