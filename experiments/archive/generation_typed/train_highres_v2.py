...
Still warm-starts the decoder/pattern/type heads + onset branch from gen_stage1 (those work); only the
audio-encoder first-conv new column is reinitialized. From cache/samples_v3 (42-dim, already warm).

Usage:
    python experiments/generation_typed/train_highres_v2.py --data_dir data/ --audio_dir data/ \
        --epochs 15 --batch_size 8 --offbeat_weight 3.0
"""
...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_highres_v2')
    p.add_argument('--cache_dir', default='cache/samples_v3', help='42-dim cache (41 musical + high-res onset)')
    p.add_argument('--offbeat_weight', type=float, default=3.0,
                   help='onset-loss weight on off-beat frames (t%%4 != 0); on-beat stays 1.0. Creates the '
                        'gradient incentive for syncopation the v1 frame-wise CE lacked.')
...
def main():
    ...
    def losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active):
        ...
        # off-beat-upweighted onset loss: off-beat frames (t%4 != 0) weigh `offbeat_weight`, on-beat 1.0.
        B, T = ol.shape
        t = torch.arange(T, device=ol.device)
        phase_w = torch.where(t % 4 != 0, float(args.offbeat_weight), 1.0).expand(B, T)
    ...
    if mlflow_on:
        mlflow.start_run(run_name="gen-highres-v2"); mlflow.log_params({'epochs': args.epochs, 'lr': args.lr, 'features': '42-dim: stage1 + high-res onset', 'warm_start': 'gen_stage1', 'offbeat_weight': args.offbeat_weight, 'new_col_init': 'random'})
