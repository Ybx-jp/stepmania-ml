...
H4-v6 (THIS script; fix 16th UNDER-COMMITMENT — see notes/chaos_retrain_scope.md). Step 0
...
def parse_args():
    ...
    p.add_argument('--checkpoint_dir', default='checkpoints/gen_highres_v6')
...
def main():
    ...
    def losses(ol, pat_l, typ_l, states, mask, pat_t, typ_t, active):
        ...
        # v6 RANKING/MARGIN loss: real-16th-note logits must out-rank the EMPTY off-beat frames they compete
        # with for the density budget, so they clear the per-song threshold (Step 0: knows-but-loses — model
        # scores real 16ths below 8ths). Hinge each real-16th logit above a sampled empty-off-beat logit.
        pos_m = mask & pos & is16                       # real 16th notes
        neg_m = mask & (~pos) & (is8 | is16)            # empty off-beat frames (the competitors)
    ...
    best = float('inf'); bad = 0; best_path = Path(args.checkpoint_dir) / "best_val.pt"  # MINIMIZE val_s16err
    if mlflow_on:
        mlflow.start_run(run_name="gen-highres-v6"); mlflow.log_params({'epochs': args.epochs, 'lr': args.lr, 'features': '42-dim + high-res onset', 'warm_start': 'gen_highres_v4', 'w8': args.w8, 'w16': args.w16, 'rank_lambda': args.rank_lambda, 'rank_margin': args.rank_margin, 'select_metric': 'val_s16err'})
    ...
    for epoch in range(args.epochs):
        ...
        s16errs = []  # per-song |gen 16th-share - real 16th-share| (DEPLOYMENT-MATCHED: coherent radar + per-song threshold)
        with torch.no_grad():
            for batch in batches(val, args.batch_size, False):
                ...
                for b in range(ol.shape[0]):
                    ...
                    nnote = int(note_b.sum())
                    if nnote < 8:
                        continue
                    ph = np.arange(len(p_b)) % 4; is16_b = (ph == 1) | (ph == 3)
                    placed = p_b > float(np.quantile(p_b, 1 - note_b.mean()))   # per-song density threshold
                    if placed.sum() == 0:
                        continue
                    gen16 = placed[is16_b].sum() / placed.sum(); real16 = (note_b & is16_b).sum() / nnote
                    s16errs.append(abs(gen16 - real16))
        v = [x / max(1, vnb) for x in v]; vt = v[0] + v[1] + v[2]
        s16err = float(np.mean(s16errs)) if s16errs else 1.0
        print(f"  epoch {epoch+1}/{args.epochs}  train(o={tr[0]/max(1,nb):.3f} pat={tr[1]/max(1,nb):.3f} typ={tr[2]/max(1,nb):.3f} rank={tr[3]/max(1,nb):.3f})  "
              f"val(o={v[0]:.3f} rank={v[3]:.3f})  s16err={s16err:.4f}" + ("  *" if s16err < best else ""))
        if mlflow_on: mlflow.log_metrics({'val_onset': v[0], 'val_pattern': v[1], 'val_type': v[2], 'val_rank': v[3],
                                          'val_total': vt, 'val_s16err': s16err}, step=epoch)
        if s16err < best:
            best = s16err; bad = 0
            torch.save({'model_state_dict': model.state_dict(), 'epoch': epoch, 'val_s16err': s16err, 'val_total': vt}, best_path)
        else:
            bad += 1
            if bad >= args.patience:
                print(f"  early stop @ epoch {epoch+1}: no val_s16err improvement for {args.patience} epochs"); break

    # ---- eval ----
    print(f"\nLoading best (val_s16err={best:.4f}) for eval...")
